...
def init_db():
    ...
    cursor = conn.cursor()
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS woolies_prices (
...
def store_price(wlref: str, product_name: str, product_id: str, full_link: str, price: float, percentage_drop: int = 0):
    ...
    if not existing_record:
        ...
        cursor.execute('''
            INSERT INTO woolies_prices (wlref, product_name, product_id, full_link, price, percentage_drop)
            VALUES (?, ?, ?, ?, ?, ?)
